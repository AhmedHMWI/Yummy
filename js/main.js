...
sideBarToggle = function () {
  isTrue
    ? ($(".navContent").addClass("open-menu").removeClass("close-menu"),
      $(".bgBlack").animate({ opacity: "1" }, 250),
      $(".ss").animate(
        {
          opacity: "1",
          position: "absolute",
      ...
      (isTrue = !isTrue))
    : ($(".bgBlack").animate({ opacity: "0" }, 250),
      $(".navContent").addClass("close-menu").removeClass("open-menu"),
      $(".ss").animate(
        {
          opacity: "0",
        },
      ...
      for (let i = 0; i < 24; i++) {
        box += `<div class="col-md-3 ">
                    <div  class="Item text-center">
                        ...
                        <h2 class="text-white">${ingredients.meals[
                          i
                        ].strIngredient.slice(0, 200)}</h2>
                        <p class="text-white">${ingredients.meals[
                          i
                        ].strDescription.slice(0, 200)}</p>
                        </div>
                        </div>`;
      }
      $(".click").html(box);
  ...
  response.addEventListener("loadend", function () {
    if (response.status == 200) {
      ...
      for (let i = 0; i < ingredients.meals.length; i++) {
        ...
        for (let k = 1; k < 21; k++) {
          if (
            ingredients.meals[i]["strIngredient" + k] != "" &&
            ingredients.meals[i]["strIngredient" + k] != null
          ) {
            ingredientsStr += `<li class="my-3 mx-1 p-1 alert alert-success rounded-1">${
              ingredients.meals[i]["strMeasure" + k]
            } ${ingredients.meals[i]["strIngredient" + k]}</li>`;
          }
        }
      ...
      for (let i = 0; i < category.categories.length; i++) {
        box += `            
                <div class="col-md-3 ">
                    <div class="hoverItem rounded-1 overflow-hidden ">
                        <img src="${
                          category.categories[i].strCategoryThumb
                        }" alt="beef" class="w-100 rounded-1 ">
                        <div class="hoverInner text-center rounded-1 p-1">
                            <h4 class="fw-lighter fa-2x">${
                              category.categories[i].strCategory
                            }</h4>
                            <p class="fw-normal">${category.categories[
                              i
                            ].strCategoryDescription.slice(0, 120)}</p>
                    </div>
                </div>
            </div>`;
      }
...
function messageInvalidName(flag) {
  if (!flag) {
    return (document.querySelector(
      "#nameError"
    ).innerHTML = `<p class="int">Please enter your Name</p>`);
  } else {
...
function messageInvalidEmail(flag) {
  if (!flag) {
    return (document.querySelector(
      "#emailError"
    ).innerHTML = `<p class="int">Please enter valid Email</p>`);
  } else {
...
function messageInvalidPhone(flag) {
  if (!flag) {
    return (document.querySelector(
      "#phoneError"
    ).innerHTML = `<p class="int">Please enter valid Phone Number Ex: 01xxxxxxxxx </p>`);
  } else {
...
function messageInvalidAge(flag) {
  if (!flag) {
    return (document.querySelector(
      "#ageError"
    ).innerHTML = `<p class="int">Please enter valid Age</p>`);
  } else {
...
function messageInvalidPassword(flag) {
  if (!flag) {
    return (document.querySelector(
      "#passwordError"
    ).innerHTML = `<p class="int">Please enter password  has a minimum of 6 characters, at least 1 uppercase letter, 1 lowercase letter, and 1 number with no spaces.</p>`);
  } else {
...
function messageInvalidConfirmPassword(flag) {
  if (!flag) {
    return (document.querySelector(
      "#passwordConfError"
    ).innerHTML = `<p class="int">Please enter same Password again</p>`);
  } else {
